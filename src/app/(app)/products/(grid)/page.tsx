...
import { PerPageLink } from './PerPageLink'
...
export default async function ProductsPage({ searchParams }: Props) {
  const { q: searchValue, sort, category, brand, page: pageParam, limit: limitParam } = await searchParams
  const brandIds = brand ? (Array.isArray(brand) ? brand : [brand]) : []

  const page = Math.max(1, parseInt(String(pageParam ?? '1'), 10))
  const limit = LIMIT_OPTIONS.includes(parseInt(String(limitParam ?? ''), 10))
  ...
  const resultsText = totalDocs === 1 ? 'result' : 'results'
    ...
    <div>
      ...
      <div className="mb-6 flex items-center justify-between gap-4">
        <p className="font-mono text-xs uppercase tracking-widest text-neutral-400">
          {totalDocs === 0
            ? 'No products found'
            : `${totalDocs} ${resultsText}${searchValue ? ` for "${searchValue}"` : ''}`}
        </p>
...
function PerPageSelector({ current }: { current: number }) {
  return (
    <div className="flex items-center gap-2">
      <span className="font-mono text-[9px] uppercase tracking-widest text-neutral-400">Show</span>
      <div className="flex gap-0 border border-neutral-200">
        {LIMIT_OPTIONS.map((opt) => (
          <PerPageLink key={opt} value={opt} active={opt === current} />
        ))}
      </div>
    </div>
  )
